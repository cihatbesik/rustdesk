...
def build_flutter_dmg(version, features):
    ...
    # eMuhasebe: PRODUCT_NAME (AppInfo.xcconfig) markalama icin degistirildigi
    # icin uretilen .app dosyasi artik "RustDesk.app" degil ("eMuhasebe
    # Destek.app"). Sabit ad yerine Release klasorundeki .app'i glob ile bul.
    release_dir = 'build/macos/Build/Products/Release'
    app_matches = glob.glob(os.path.join(release_dir, '*.app'))
    if len(app_matches) != 1:
        raise Exception(
            f'expected exactly one .app in {release_dir}, found {len(app_matches)}: {app_matches}')
    app_bundle_dir = app_matches[0]
    system2(f'cp -rf ../target/release/service "{app_bundle_dir}/Contents/MacOS/"')
    '''
    system2(
